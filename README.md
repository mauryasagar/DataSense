...
**DataSense** is a fully browser-sandboxed workspace. It enables local analysis of CSV sheets, PDF document Q&A, and Jupyter Notebook cell explanation using client-side AI runtimes via WebAssembly. Raw data is kept strictly private on the user's machine — it never leaves the browser tab.

Key capabilities:
- **CSV Data Chat** — Natural language queries answered by a local NLQ engine (zero latency for math/stats) with AI fallback
- **EDA Copilot** — Instant exploratory data analysis: distributions, outliers, correlations, missing values
...
1. **Target Hardware**:
   - Runs locally in modern web browsers (Chrome 112+, Edge, Firefox, Safari) using client CPU/GPU via WebAssembly (WASM SIMD).

2. **AI Models & Framework**:
   - **Framework**: Transformers.js (v2) utilizing ONNX Runtime Web.
   - **Unified Chat Model**: [Xenova/Qwen1.5-0.5B-Chat](https://huggingface.co/Xenova/Qwen1.5-0.5B-Chat) (Apache-2.0 License).

3. **Model Footprint**:
   - **Baseline Size**: ~950 MB (FP16/FP32).
   - **Quantized Footprint**: ~350 MB (ONNX format, int8 quantized weights).
   - **RAM Usage During Inference**: ~800 MB – 1 GB.
...
4. **Performance Metrics (Average Desktop/Laptop CPU)**:
   - **First-time model download**: ~5–15 minutes depending on connection speed (~350 MB).
   - **WASM initialization / warm-up** (from cache, on page load): ~15–30 seconds on CPU-only devices. Click **"Initialize AI"** when ready — the app is fully functional for data exploration without waiting.
   - **NLQ Engine (local math/stats queries)**: ~0 ms — computed instantly without the AI model.
   - **CSV chat response time (AI-assisted)**: ~1.5s – 4s.
   - **PDF document Q&A response time**: ~1.5s – 3s.
   - **Generative text summarization latency**: ~2s – 5s.
   - **Generation Rate**: ~8–16 tokens/sec (CPU, quantized model).
...
3. Start the development server:
   ...
   npm run dev
   ```
4. Open [`http://localhost:5173`](http://localhost:5173) in your browser (Chrome recommended for best WASM performance).
5. Upload a CSV, PDF, or `.ipynb` file to begin analysis immediately — no AI model needed for data exploration.
6. When you need AI features (Q&A, summaries, notebook explanations), click **"Initialize AI"** or **"Load AI Models"** in the banner. This is a one-time ~350 MB download that is then cached locally for offline use.
...
- **Model Weights**: The Qwen1.5-0.5B-Chat model is licensed under the [Apache 2.0 License](https://huggingface.co/Xenova/Qwen1.5-0.5B-Chat) by the Alibaba Group.